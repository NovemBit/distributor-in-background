...
//		$btm_bulk_arg = new \BTM_Task_Bulk_Argument( [ $posts ], -10 );
//		\BTM_Task_Manager::get_instance()->register_task( $btm_task, [ $btm_bulk_arg ] );
		\BTM_Task_Manager::get_instance()->register_task_bulk( $btm_task, $posts );
	...
	if ( isset( $status['data'] ) && is_array( $status['data'] ) ) {
		foreach ( $status['data'] as $post_id => $result ) {
			if ('success' === $result['status'] ) {
				...
				$task_run_filter_log->add_log( 'Failded to fixed post having id: ' . $post_id );
				$is_failed = true;
			}
		}
	}
