...
function setup() {
	add_action(
		...
		function () {
			...
			add_filter( 'dt_allow_wc_variations_insert', __NAMESPACE__ . '\schedule_variation_insert', 10, 3 );
...
function schedule_variation_insert( $variation_processing_allowed, $post_id, $remote_post_id, $signature, $target_url ) {
	if ( \DT\NbAddon\DTInBackground\Helpers\is_btm_active() ) {
		$btm_task = new \BTM_Task( 'wc_variations_in_bg', [ $post_id, $remote_post_id, $signature, $target_url ], 10 );
